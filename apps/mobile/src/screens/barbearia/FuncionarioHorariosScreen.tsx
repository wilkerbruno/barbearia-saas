...
// Igual à tela "Meus horários" do próprio funcionário (screens/funcionario/HorariosScreen),
// mas usada pelo dono da barbearia pra editar a agenda de QUALQUER funcionário
// da equipe (rotas BARBEARIA_ADMIN em /funcionarios/:id/horarios e
// /funcionarios/:id/folgas — ver FuncionariosController). Chegada aqui a
// partir do card do funcionário na tela Equipe.
export function FuncionarioHorariosScreen({ route }: Props) {
  const { funcionarioId, nome } = route.params;
  ...
  const carregar = useCallback(async () => {
    const [horariosRes, folgasRes] = await Promise.all([
      api.get<HorarioTrabalho[]>(`/funcionarios/${funcionarioId}/horarios`),
      api.get<Folga[]>(`/funcionarios/${funcionarioId}/folgas`),
    ]);
    setDias((atual) =>
      atual.map((dia) => {
  ...
  async function salvarFolga() {
    ...
    // agenda do app, evita depender do fuso configurado no aparelho.
    ...
    try {
      await api.post(`/funcionarios/${funcionarioId}/folgas`, { inicio, fim, motivo: novaFolga.motivo.trim() || undefined });
    ...
    <SafeAreaView style={styles.container} edges={["top"]}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.secaoFolgas}>
          ...
          <Text style={styles.subtitle}>
            Use aqui pra bloquear um período específico além do expediente normal (folga, consulta, férias etc).
          </Text>
